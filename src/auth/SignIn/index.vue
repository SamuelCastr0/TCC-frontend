<template>
    <Form class="form" @submit="onSubmit" :validation-schema="schema" v-slot="{ meta, isSubmitting }">
      ...
      <Input name="password" type="password" label="Senha" placeholder="suaSenhaSecreta123" />
      <Button :disabled="!meta.valid || isSubmitting" :is-loading="isSubmitting">Entrar</Button>
...
<script setup lang="ts">
  ...
  const schema = yup.object({
    ...
    password: yup.string().required('Senha obrigatória').min(8, 'Precisa ter no mínimo 8 caracteres'),
  });
